...
myForm.addEventListener("submit", function (e)
{
  ...
  reader.onload = function (e) {
    ...
    const arrOfNumbers = [];//(Array will holds population)
     ...
     writeOnFile(arrOfStates,MinimumRep,data,filename)
     //Print
...
//Using Hamilton's Apportionment Algorithm
...
function writeOnFile(arrOfStates,MinimumRep,data,filename)
{
  const copyArrOfStates=[];
for(let i=0; i<arrOfStates.length; i++)
{
...
for(let i=0; i<arrOfStates.length; i++)
{
  let value=copyArrOfStates[i];
  let index=arrOfStates.indexOf(value);
  data+=arrOfStates[index]+"   "+MinimumRep[index]+" \n";
}
// let txtFile =new File(filename);
//   txtFile.writeln(data);
//   txtFile.close();
}
